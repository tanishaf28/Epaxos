...
func NewEPaxosManager(serverID, numReplicas int) *EPaxosManager {
	...
	go mgr.startDeadReplicaReviver(3 * time.Second)
...
func (m *EPaxosManager) runFastPath(instanceID InstanceID, cmd *Command) (bool, string, time.Time) {
	...
	m.state.AtomicPreAccept(func() {
		...
		m.state.RegisterObjectAccess(instanceID, objIDs, cmd.CmdType)
	...
	// Mirrors efficient/epaxos's inst.lb.committedDeps (epaxos.go:1036-1047):
	// merged max of every reply's CommittedUpTo plus our own, checked against
	// the proposed deps before allowing a fast-path commit.
	m.RLock()
	committedUpToSnapshot := make([]int32, numOfServers)
	copy(committedUpToSnapshot, m.committedUpTo)
	m.RUnlock()
	...
	for reply := range responses {
		if !reply.OK {
			...
			mergeCommittedUpTo(committedUpToSnapshot, reply.CommittedUpTo)
		} else {
	...
	// allCommitted: every dependency in the proposed set must already be known
	// committed by at least one replying replica (efficient/epaxos epaxos.go:1044-1046).
	allCommitted := true
	for replicaID, instNo := range deps {
		if instNo >= 0 && instNo > committedUpToSnapshot[replicaID] {
			allCommitted = false
			break
		}
	}
	...
	log.Debugf("[FastPath] Cannot commit | Instance=%s | Reason: MatchingReplies=%d (need %d) OR allEqual=%v OR allCommitted=%v",
		instanceID, matchCount, fastQuorum, allEqual, allCommitted)

	return false, "FAST", start
}

// mergeCommittedUpTo folds a PreAcceptReply's CommittedUpTo view into the
// leader's running snapshot, element-wise max (efficient/epaxos epaxos.go:1038-1043).
func mergeCommittedUpTo(snapshot []int32, replyView []int32) {
	for i := 0; i < len(snapshot) && i < len(replyView); i++ {
		if replyView[i] > snapshot[i] {
			snapshot[i] = replyView[i]
		}
	}
}
...
func (m *EPaxosManager) runSlowPath(instanceID InstanceID, cmd *Command, fastPathStart time.Time) (bool, string) {
	...
	// Was 500ms -- too tight for large batches (BATCHSIZE=1000+) under added
	// network delay: serialization/transmission time alone can approach or
	// exceed 500ms per hop, causing the Accept round to time out and the
	// whole command to fail consensus even though replicas were healthy.
	// Bumped to match the 5s timeout already used elsewhere in this file
	// (e.g. the PreAccept fast path's related deadline).
...
// reviveReplica un-marks a replica as dead so PreAccept/Accept broadcasts
// include it again. Without this, markReplicaDead is permanent for the life
// of the process: dead replicas are skipped in the broadcast loops (see
// isReplicaDead call sites), so trackReplicaSuccess/trackReplicaFailure never
// fire for them again and the mark can never clear on its own.
func (m *EPaxosManager) reviveReplica(replicaID int) {
	m.deadReplicasMu.Lock()
	wasDead := m.deadReplicas[replicaID]
	delete(m.deadReplicas, replicaID)
	m.deadReplicasMu.Unlock()
	if wasDead {
		log.Warnf("[CRASH-DETECT] Replica %d responded to health check - un-marked as dead", replicaID)
	}
...
// startDeadReplicaReviver periodically pings every replica currently marked
// dead and revives any that respond, so a restarted replica rejoins
// PreAccept/Accept broadcasts instead of staying excluded until this
// process is also restarted.
func (m *EPaxosManager) startDeadReplicaReviver(interval time.Duration) {
	ticker := time.NewTicker(interval)
	defer ticker.Stop()
	for range ticker.C {
		m.deadReplicasMu.RLock()
		dead := make([]int, 0, len(m.deadReplicas))
		for id := range m.deadReplicas {
			dead = append(dead, id)
		}
		m.deadReplicasMu.RUnlock()
		...
		for _, id := range dead {
			...
			reply := &ClientReply{}
			if err := conn.rpcClient.Call("EPaxosService.Ping", &ClientArgs{CmdType: READ}, reply); err == nil {
				m.reviveReplica(id)
			}
		}
	}
