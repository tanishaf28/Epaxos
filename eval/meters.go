...
func (m *PerfMeter) SaveToFile() error {
	...
	// Track the earliest start and latest end across all batches that
	// actually finished (TimeElapsed != 0) for the wall-clock throughput
	// window below. This must be a true min/max, not "keys[0]/keys[last]
	// in globalClock order" -- incrementing the globalClock counter and
	// calling RecordStarter's time.Now() aren't atomic together, so under
	// concurrency a goroutine can grab a lower globalClock but call
	// time.Now() later than a goroutine holding a higher one (and
	// keys[0]/keys[last] aren't even guaranteed to have finished). Using
	// positional indexing let the window invert, silently zeroing out
	// throughput instead of reporting the real value.
	...
	for _, key := range keys {
		...
		end := rec.StartTime.Add(time.Duration(rec.TimeElapsed) * time.Millisecond)
		if earliestStart.IsZero() || rec.StartTime.Before(earliestStart) {
			earliestStart = rec.StartTime
		}
	...
	wallClockSecs := latestEnd.Sub(earliestStart).Seconds()
