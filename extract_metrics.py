...
n11_eval1_ratio_0.0) or, for client-scaling sweeps, "clients_<N>" (e.g.
clients_10), so the case label is the CSV's parent directory name, and
size is parsed from that prefix.
...
def _pick_latest(csvs: list[str]) -> str:
    ...
    # merged_{clients,servers}_<YYYYMMDD_HHMMSS>.csv); mtime instead reflects
    # when archive_latest_result() happened to cp the file, which can lag or
    # batch unrelated stale files together (e.g. a missing .last_archive_ts
    # marker on the first case makes `find -newer` match everything).
