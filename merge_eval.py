#!/usr/bin/env python3
"""Merge per-client or per-server EPaxos evaluation CSV files into summary CSVs.

Usage:
    ./merge_eval.py [eval_dir] [output]           # Merge client CSVs (default)
    ./merge_eval.py [eval_dir] [output] --servers # Merge server CSVs

Examples:
    ./merge_eval.py
    ./merge_eval.py ./eval
    ./merge_eval.py ./eval ./eval/merged/merged_clients.csv
    ./merge_eval.py ./eval ./eval/merged/merged_servers.csv --servers
"""
...
def _extract_run_timestamp(path: str) -> Optional[str]:
    ...
    # clientX_epaxos_YYYYMMDD_HHMMSS.csv
...
def _pick_latest_client_csv(csvs: list[str]) -> str:
...
def merge_client_csvs(
        ...
        with open(latest, "r", newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if label == "THROUGHPUT":
                    ...
                    if val is not None:
                        total_conflict += val
                elif label == "TOTAL_TIMEOUT_COMMITS":
                    val = _parse_int(_cell(row, 1))
                    if val is not None:
                        total_timeout += val
                elif label == "TOTAL_NETWORK_ERRORS":
                    val = _parse_int(_cell(row, 1))
                    if val is not None:
                        total_network_errors += val
    ...
    with open(output_path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writerow(["TOTAL_TIMEOUT_COMMITS", total_timeout, ""])
        writer.writerow(["TOTAL_NETWORK_ERRORS", total_network_errors, ""])
...
def merge_server_csvs(
    ...
    """Merge per-server EPaxos evaluation CSV files into one summary CSV.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Merge all latest per-client or per-server evaluation CSV files into a summary CSV"
        )
    )
    parser.add_argument(
        "eval_dir",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: ./eval/merged/merged_<type>_<timestamp>.csv"
        ),
    )
    parser.add_argument(
        "--servers",
