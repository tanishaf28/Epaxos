...
// ClassifyRealKey deterministically types a real (non-synthetic) YCSB key as
// independent or dependent, driven by the same -indep ratio that controls
// the synthetic plain-msg pool's split — mirrors woc/objectmap.go's
// classifyRealKey exactly. Hashing the key (rather than rolling a fresh
// random number per access) keeps the type static per key: every client and
// replica computes the same type for the same key without coordination, and
// the real key is never substituted, so YCSB's actual Zipfian/latest access
// distribution and scan semantics are untouched.
func ClassifyRealKey(key string, indepRatio float64) int {
	if key == "" {
