#!/bin/bash
# ================================================================
# HETERO-5 NETEM RATIO SWEEP + TIMELINE (EPaxos): independent-ratio
# sweep (100 -> 0) under a fixed server-side-only netem delay profile,
# with a 5-client pool and BATCHSIZE=100, followed by a single
# INDEP_RATIO=90 timeline run (ENABLE_TIMESERIES=true).
#
# Adapted from run_hetero5_netem_i2d_sweep.sh: NUM_CLIENTS 2->5 (needs
# the 10-client-pool config, since the default 5n_2s_3w.conf only has 2
# client slots), BATCHSIZE 1->100, JITTER_MS 5->10 (+-10ms), plus a
# trailing timeline-only case.
#
...
echo "================================================================"
echo " HETERO-5 NETEM RATIO SWEEP + TIMELINE (EPaxos): ${DELAY_MS}ms +-${JITTER_MS}ms, 5 clients, batch=100"
echo "================================================================"
...
echo ""
echo "=================================================="
echo " Hetero-5 netem ratio sweep + timeline complete (EPaxos)"
echo "=================================================="
