#!/bin/bash
# ================================================================
# EVAL 5 (EPaxos port): Indep-to-Dependent Ratio Sweep x Cluster Size,
# 10-client pool
#
# Same sweep as run_hetero_plainmsg_evals.sh's eval1 (INDEP_RATIO over
# 100,90,80,60,40,20,10,0, batch=1, msgsize=512), across cluster sizes
...
# heterogeneous configs, mirroring WOC's eval_5_i2d_size_sweep.sh.
#
# Added alongside, not replacing, run_hetero_plainmsg_evals.sh's eval1 --
...
RESULT_ROOT="${SCRIPT_DIR}/results/hetero_ratio_sweep_10c"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_hetero_ratio_sweep_10c.sh

Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 (matches WOC's
eval_5_i2d_size_sweep.sh / this repo's run_hetero_plainmsg_evals.sh eval1)
with BATCHSIZE=1, MSG_SIZE=512 fixed, across cluster sizes n=3,5,7,11,
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║     RATIO SWEEP x CLUSTER SIZE, 10-CLIENT POOL (EPaxos)        ║"
...
echo ""
echo "=================================================="
echo " Ratio sweep (10-client pool) complete"
echo "=================================================="
