#!/bin/bash
# ================================================================
# EVAL 5b (EPaxos port): Indep-to-Dependent Ratio Sweep x Cluster Size,
# 10-client pool, READ_RATIO fixed at 100 (all reads)
#
# Read-heavy counterpart of run_hetero_ratio_sweep_10c.sh: identical sweep
# (INDEP_RATIO over 100,90,80,60,40,20,10,0, batch=1, msgsize=512,
# n=3,5,7,11, dedicated 10-VM client pool) but with READ_RATIO=100.0
# fixed for every case instead of the default 0.0 (all writes), mirroring
# woc/cabinet/raft's read100 counterparts for direct cross-system
# comparison.
# ================================================================
...
RESULT_ROOT="${SCRIPT_DIR}/results/hetero_ratio_sweep_10c_read100"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_hetero_ratio_sweep_10c_read100.sh

Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 with READ_RATIO fixed at
100.0 (all reads), BATCHSIZE=1, MSG_SIZE=512 fixed, across cluster sizes
...
Results archived under: results/hetero_ratio_sweep_10c_read100/<timestamp>/<label>/
EOF
    exit 0
fi
...
run_case() {
    ...
    # Raw per-node CSVs accumulate in eval/client*/eval/server* forever
    # otherwise (each case adds new timestamped files, nothing here ever
    # deletes old ones) -- already archived above, so safe to clear now.
    rm -rf "${SCRIPT_DIR}"/eval/client* "${SCRIPT_DIR}"/eval/server* 2>/dev/null || true
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  RATIO SWEEP x CLUSTER SIZE, 10-CLIENT POOL (EPaxos, read=100%)║"
...
for n in "${ALL_CLUSTER_SIZES[@]}"; do
    ...
    for indep in "${TEST_CASES[@]}"; do
        BASE_ENV=(
            ...
            "INDEP_RATIO=${indep}" "NUM_OBJECTS=1000" "READ_RATIO=100.0"
...
echo ""
echo "=================================================="
echo " Ratio sweep (10-client pool, read=100%) complete"
echo "=================================================="
