#!/bin/bash
# ================================================================
# MongoDB Batch Size Sweep x Cluster Size (EPaxos)
#
# Sweeps BATCHSIZE over 1,10,50,100,500,1000,2000 (same points as
# run_mongodb_batchsize_sweep_5n.sh / run_hetero_plainmsg_evals.sh's
# eval2) for each cluster size in CLUSTER_SIZES (3,5,7,11 servers),
# against the MongoDB-backed EPaxos cluster (start_mongodb_hetero_nsel.sh,
# -et=1). INDEP_RATIO=90.0 and MSG_SIZE=512 fixed throughout, matching
# run_mongodb_ratio_sweep_nsel.sh's sibling script.
#
# -et=1 is hardcoded inside start_mongodb_hetero_nsel.sh itself (not an
# env override here) -- every case this script runs is MongoDB-backed.
# ================================================================
...
run_case() {
    ...
    echo "=================================================="
    echo "Running: $label"
    echo "  n=${CURRENT_N}  et=1 (mongodb)  batch=${batch_size}  indep=90.0  workload=${WORKLOAD}  runtime=${RUNTIME_SECONDS}s"
    echo "=================================================="
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  EPAXOS MONGODB BATCH SIZE SWEEP x CLUSTER SIZE (n=3,5,7,11)     ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
for n in "${ALL_CLUSTER_SIZES[@]}"; do
    ...
    for batch_size in "${TEST_CASES[@]}"; do
        run_case "n${n}_epaxos_mongo_batch_${batch_size}" "$batch_size"
    done
done
...
echo ""
echo "=================================================="
echo " EPaxos MongoDB batch size sweep x cluster size complete"
echo "=================================================="
