#!/bin/bash
# ================================================================
# MongoDB Network Delay Eval (EPaxos), n=5 heterogeneous, with timeseries
#
# EPaxos previously had NO MongoDB delay eval at all. This ports WOC's
# run_mongodb_delay_eval_5n.sh: sweeps injected server-egress delay
# (0,5,10,20,50,100,200ms, via tc qdisc netem - see
# run_hetero5_netem_eval.sh for the same mechanism and its "server-side
# only" caveat: delay lands on each server's own egress interface, which
# also carries replies back to clients) against the EPaxos MongoDB
# cluster (start_mongodb_hetero_5n.sh, -et=1, fixed n=5, unchanged from
# the existing n=5-only start/stop scripts - not the new nsel scripts).
#
# ENABLE_TIMESERIES=true throughout, using the same collection pattern as
# WOC's run_hetero5_crash_eval.sh (the one existing eval script that
# already does this correctly): client.go writes
# tps_timeline_<clientID>.csv into its own eval dir as it runs; after
# each case, tps_timeline_*.csv files newer than this run's start marker
# are copied straight into that case's result dir (not nested under
# merged/), which is exactly the layout plot_timeseries.py's
# find_timeline_csvs() scans for. plot_timeseries.py is invoked
# automatically at the end against the whole RUN_DIR, so every case with
# a valid timeseries gets its own throughput-over-time plot without a
...
# n=5 heterogeneous server pool (matches start_mongodb_hetero_5n.sh /
# stop_mongodb_hetero_5n.sh's SERVER_IPS - cluster_hetero_mongo_5n.conf's
# first 5 IPs).
SERVER_IPS=(
    "192.168.73.59"
    "192.168.73.243"
...
# the ratio/threshold-baseline/workload sweeps that don't collect
# timeseries).
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║  EPAXOS MONGODB DELAY EVAL, n=5 (server-side only, with timeseries) ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
echo ""
echo "=================================================="
echo " EPaxos MongoDB delay eval complete"
echo "=================================================="
