#!/bin/bash
# ================================================================
# MongoDB EVAL 1 (EPaxos port): Indep-to-Dependent Ratio Sweep,
# HETEROGENEOUS 5-NODE (2 Strong + 3 Weak) MongoDB replica set.
#
# Same sweep as run_hetero_plainmsg_evals.sh's eval1 (INDEP_RATIO over
# 100,90,80,60,40,20,10,0, batch=1, msgsize=512), but against the
# MongoDB-backed cluster (start_mongodb_hetero_5n.sh / -et=1) instead of
# the plain-msg cluster (start_cluster_hetero.sh / -et=0). Fixed at n=5
# because the MongoDB launcher's SERVER_IPS pool is 5 specific hosts.
# ================================================================
...
USER="ubuntu"
RESULT_ROOT="${SCRIPT_DIR}/results/mongodb_ratio_sweep_5n"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_mongodb_ratio_sweep_5n.sh

Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 (matches
run_hetero_plainmsg_evals.sh's eval1) with BATCHSIZE=1, MSG_SIZE=512
...
Results archived under: results/mongodb_ratio_sweep_5n/<timestamp>/<label>/
EOF
    exit 0
fi
...
run_case() {
    ...
    echo "=================================================="
    echo "Running: $label"
    echo "  n=5 (2s+3w)  indep=${indep}  workload=${WORKLOAD}  runtime=${RUNTIME_SECONDS}s"
    echo "=================================================="

    CLUSTER_ACTIVE=true
    INDEP_RATIO="$indep" BATCHSIZE=1 MSG_SIZE=512 NUM_OBJECTS=1000 READ_RATIO=0.0 \
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   MONGODB RATIO SWEEP, HETEROGENEOUS 5-NODE (2s+3w) EPaxos     ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
for indep in "${TEST_CASES[@]}"; do
    run_case "n5_mongo_ratio_${indep}" "$indep"
done
...
echo ""
echo "=================================================="
echo " MongoDB ratio sweep complete"
echo "=================================================="
