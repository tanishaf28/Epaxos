#!/bin/bash
# ================================================================
# MongoDB Workload Sweep (EPaxos port), HETEROGENEOUS 5-NODE
# (2 Strong + 3 Weak), standalone mongod per node.
#
# Sweeps YCSB WORKLOAD over a,b,c,d,e,f with INDEP_RATIO fixed at 90/10,
# BATCHSIZE=1, MSG_SIZE=512, against the MongoDB-backed cluster
# (start_mongodb_hetero_5n.sh / -et=1). Mirrors
# run_mongodb_ratio_sweep_5n.sh but sweeps the orthogonal workload-type
# dimension instead of the indep/dependent ratio.
#
# ENABLE_TIMESERIES=true throughout, using the same collection pattern as
# run_hetero5_crash_eval.sh / run_mongodb_delay_eval_5n.sh: client.go
# writes tps_timeline_<clientID>.csv into its own eval dir as it runs;
# after each case, tps_timeline_*.csv files newer than this run's
# last-archive marker are copied straight into that case's result dir
# (not nested under merged/), which is exactly the layout
# plot_timeseries.py's find_timeline_csvs() scans for. plot_timeseries.py
# is invoked automatically at the end against the whole RUN_DIR, so each
# workload case gets its own throughput-over-time plot without a manual
# follow-up step. RUNTIME_SECONDS default raised from 30 to 60 -
# timeseries evals need more samples than a plain throughput/latency run.
# ================================================================
...
STOP_SCRIPT="${SCRIPT_DIR}/stop_mongodb_hetero_5n.sh"
...
# Must match start_mongodb_hetero_5n.sh's SERVER_IPS / CLIENT_HOST_IPS --
...
# first-few-seconds ramp-up - raised from 30 to 60 (matches
# run_mongodb_delay_eval_5n.sh).
...
BATCHSIZE=1, MSG_SIZE=512, against the heterogeneous 5-node (2 strong +
3 weak) MongoDB cluster (start_mongodb_hetero_5n.sh, -et=1).
...
Timeseries plots: results/mongodb_workload_sweep_5n/<timestamp>/plots/
EOF
    exit 0
fi
...
cleanup_all_nodes() {
    echo "=================================================="
    echo " Pre-sweep cleanup: purging stale epaxos/mongod processes..."
    echo "=================================================="
    for ip in "${ALL_NODE_IPS[@]}"; do
        ssh -o ConnectTimeout=5 -i "$SSH_KEY" "$USER@$ip" \
            "pkill -9 -x epaxos 2>/dev/null; pkill -9 -x mongod 2>/dev/null" &
    done
    wait
}
...
archive_latest_result() {
    ...
    # Timeseries CSVs go directly into dest_dir (not merged/) - this is
    # the layout plot_timeseries.py's find_timeline_csvs() scans for.
...
run_case() {
    ...
    CLUSTER_ACTIVE=true
    ENABLE_TIMESERIES=true TPS_TIMELINE_INTERVAL_MS="$TPS_TIMELINE_INTERVAL_MS" \
        INDEP_RATIO="$INDEP_RATIO_FIXED" BATCHSIZE=1 MSG_SIZE=512 NUM_OBJECTS=1000 READ_RATIO=0.0 \
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   MONGODB WORKLOAD SWEEP, HETEROGENEOUS 5-NODE (2s+3w) EPaxos  ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
for workload in "${WORKLOADS[@]}"; do
    run_case "n5_epaxos_mongo_workload_${workload}" "$workload"
done
