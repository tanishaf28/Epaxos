...
#
# Wraps run_hetero_plainmsg_evals.sh with CLUSTER_SIZES="5" so the full
# n=3/7/11 sweep is skipped. Use this to sanity-check the cluster + client
# pinning on a single cluster size before committing to the full sweep.
# ================================================================

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUN_SCRIPT="${SCRIPT_DIR}/run_hetero_plainmsg_evals.sh"

export CLUSTER_SIZES="5"
export RUNTIME_SECONDS="${RUNTIME_SECONDS:-30}"

echo "################################################################"
echo "# EPaxos N=5 test — FIXED clients (FIXED_CLIENT_COUNT=3)"
echo "################################################################"
...
echo "################################################################"
echo "# EPaxos N=5 test — SCALE clients (5 pinned clients)"
echo "################################################################"
SCALE_CLIENTS=true bash "$RUN_SCRIPT" eval1
SCALE_CLIENTS=true bash "$RUN_SCRIPT" eval2

echo ""
echo "N=5 test complete. Results under: ${SCRIPT_DIR}/results/hetero_plainmsg_evals/<timestamp>/n5/"
