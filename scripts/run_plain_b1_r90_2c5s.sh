#!/bin/bash
# ================================================================
# HETERO-5 PLAIN STEADY-STATE RUN (EPaxos): NUM_SERVERS=5, NUM_CLIENTS=2,
...
RESULT_ROOT="${SCRIPT_DIR}/results/hetero5_plain_b1_r90_2c5s"
...
cleanup() {
    if [ "$CLUSTER_ACTIVE" = true ]; then
        CLIENT_COUNT=2 CONFIG_PATH="${CONFIG_PATH}" bash "$STOP_SCRIPT" || true
    fi
}
trap cleanup EXIT

echo "================================================================"
echo " HETERO-5 PLAIN STEADY-STATE RUN (EPaxos): indep=90, batch=1, maxinflight=5, 5s/2c"
echo "================================================================"
...
CLIENT_COUNT=2 CONFIG_PATH="${CONFIG_PATH}" bash "$STOP_SCRIPT"
CLUSTER_ACTIVE=false

archive_case "plain_indep90_b1_2c5s"
