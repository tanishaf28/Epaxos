...
SERVER_IPS=(
"192.168.73.59"   # cora-c32-tani-2 (strong) -- shared with woc/cabinet/raft
"192.168.73.243"  # cora-c32-tani-1 (strong) -- shared with woc/cabinet/raft
"192.168.73.117"
"192.168.73.16"
"192.168.73.94"
)
...
copy_eval_dir() {
    ...
    ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -i "$SSH_KEY" \
        "$USER@$ip" "rm -f ${REMOTE_EVAL_DIR}/${remote_subdir}/*.csv" >/dev/null 2>&1 || true
...
echo "=================================================="
echo " EPaxos MongoDB 5-NODE Cluster Shutdown"
...
if [ "$any_left" = false ]; then
    echo " All epaxos processes stopped on all nodes."
else
...
if [ -f "${MERGE_SCRIPT}" ]; then
    if python3 "${MERGE_SCRIPT}" "${LOCAL_EVAL_DIR}" "${MERGED_DIR}/" --servers --ids "${SERVER_ID_FILTER}"; then
        ...
        echo " ✗ Error merging server CSVs (expected -- EPaxos servers don't write per-run CSVs)"
    fi
else
    echo " ✗ merge_eval.py not found at ${MERGE_SCRIPT}"
fi
