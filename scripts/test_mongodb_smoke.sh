...
# start, EPaxos never connecting to Mongo, zero documents actually
# written, etc.) in under a minute instead of discovering it 20 cases into
# a sweep. Mirrors woc/scripts/test_mongodb_smoke.sh.
#
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║           EPAXOS MONGODB SMOKE TEST (n=3, ~30s total)            ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
...
for i in "${!SERVER_IPS[@]}"; do
    ip="${SERVER_IPS[$i]}"
    # dbName convention from conns.go's initMongoDB: EPaxos branches on
...
echo "=================================================="
# Soft pre-check only - EPaxos's client log format ("[Client N] Batch %d |
# inflight=... fast=... slow=...") may not have fired yet on a short run
# depending on pipelining/batch cadence. CHECK 5 (merged CSV) is
# authoritative.
