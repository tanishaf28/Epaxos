...
func (s *EPaxosService) PreAccept(args *PreAcceptArgs, reply *PreAcceptReply) error {
	...
	epaxosMgr.state.AtomicPreAccept(func() {
		...
		epaxosMgr.state.RegisterObjectAccess(args.InstanceID, objIDs, args.Command.CmdType)
...
func (s *EPaxosService) ConsensusService(args *ClientArgs, reply *ClientReply) error {
	...
	// Determine batch size. Reads carry no CmdPlain/CmdMongo payload (see
	// client.go's isRead skip -- matches WOC/Cabinet and real EPaxos GETs,
	// which need only the key, not a value), so for reads fall back to
	// ObjIDs, same as Cabinet's validateClientRequestPayload does.
	batchSize := 0
	if args.CmdType == READ {
		batchSize = len(args.ObjIDs)
	} else if args.Type == PlainMsg {
