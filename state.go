...
type EPaxosState struct {
	...
	// Two tables, not one, so the read/write interference rule (write-write
	// and write-read conflict, read-read does not) is actually enforced
	// rather than treating every access as a write:
	//   conflicts[r][key]      = highest instance from replica r touching
	//                            key, of ANY type — what a WRITE must
	//                            depend on, since writes conflict with
	//                            everything.
	//   writeConflicts[r][key] = highest WRITE-only instance from replica r
	//                            touching key — what a READ must depend
	//                            on; reads don't need to order against
	//                            other reads.
	// Both are O(1) lookup per key per replica, not O(all instances).
	conflicts      []map[string]int32 // conflicts[replicaID][objKey] = instanceNo (any access)
	writeConflicts []map[string]int32 // writeConflicts[replicaID][objKey] = instanceNo (write-only)
}
...
func NewEPaxosState(myID int, numReplicas int) *EPaxosState {
	state := &EPaxosState{
		...
		writeConflicts: make([]map[string]int32, numReplicas),
	}
	...
	for i := 0; i < numReplicas; i++ {
		...
		state.writeConflicts[i] = make(map[string]int32)
...
// RegisterObjectAccess tracks object access by instance using per-key per-replica conflict tables.
// cmdType matters: every access updates the "any" table (a future WRITE
// must depend on it), but only WRITE accesses update the write-only table
// (a future READ only needs to depend on writes — reads don't conflict
// with other reads).
func (s *EPaxosState) RegisterObjectAccess(instanceID InstanceID, objIDs []string, cmdType CmdType) {
	s.Lock()
	defer s.Unlock()
	...
	for _, objID := range objIDs {
		...
		if cmdType == WRITE {
			if int32(instanceID.InstanceNo) > s.writeConflicts[instanceID.ReplicaID][objID] {
				s.writeConflicts[instanceID.ReplicaID][objID] = int32(instanceID.InstanceNo)
			}
		}
	}
}
...
// A WRITE depends on the latest access of any kind (it conflicts with both
// reads and writes); a READ only depends on the latest WRITE (read-read
// pairs don't conflict). Without this distinction, every READ would falsely
// serialize against other concurrent READs.
...
func (s *EPaxosState) GetInterferingInstances(cmd *Command) Dependencies {
	...
	for _, objID := range objIDs {
		for replicaID := 0; replicaID < s.numReplicas; replicaID++ {
			if instNo, exists := table[replicaID][objID]; exists {
